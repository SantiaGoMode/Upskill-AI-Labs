...
type Tab = "personalize" | "path" | "studio" | "governance" | "ledger";
...
export function PhaseTwoConsole({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="phase2-backdrop"><section className="phase2-console" role="dialog" aria-modal="true" aria-labelledby="phase2-title">
    ...
    <nav aria-label="Phase 2 areas">{(["personalize", "path", "studio", "governance", "ledger"] as Tab[]).map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item === "personalize" ? "Bring your own job" : item === "path" ? "My pathway" : item === "studio" ? "Trainer Studio" : item === "governance" ? "Governance" : "Capability Ledger"}</button>)}</nav>
